...
void game_loop(void)
{
	...
	{	
		...
		if ( (joypad_1&LEFT) && (sprites[0].x>10) && (c5<254))
		{
			...
			if ((timer&3)==0)
			{
				if (sprites[0].pattern!=(13<<2))
				{
					sprites[0].pattern = (13<<2);
					sprites[1].pattern = (14<<2);
					sprites[2].pattern = (21<<2);
					sprites[3].pattern = (22<<2);										
				}
				else
				{
					sprites[0].pattern = (15<<2);
					sprites[1].pattern = (16<<2);				
					sprites[2].pattern = (21<<2);
					sprites[3].pattern = (22<<2);										
				}
			}
...
void initLevel(byte l)
{
	...
	if ((l%4)==0) LoadPatternAndshowAScreenDan0(FOND5_PATTERNDAN0,FOND5_COLORDAN0,FOND5_NAMEDAN0,1);
	else if ((l%3)==0) LoadPatternAndshowAScreenDan0(FOND4_PATTERNDAN0,FOND4_COLORDAN0,FOND4_NAMEDAN0,1);
	//else if ((l%3)==0) LoadPatternAndshowAScreenDan0(FOND3_PATTERNDAN0,FOND3_COLORDAN0,FOND3_NAMEDAN0,1);
	else if ((l%2)==0) LoadPatternAndshowAScreenDan0(FOND2_PATTERNDAN0,FOND2_COLORDAN0,FOND2_NAMEDAN0,1);
	else if ((l%1)==0) LoadPatternAndshowAScreenDan0(FOND1_PATTERNDAN0,FOND1_COLORDAN0,FOND1_NAMEDAN0,1);
...
{	
	...
	while(1)
	{
		...
		if (workflow==W_INGAME)
		{
			...
			player.x=128;
			player.y=192-30-8;			
			sprites[0].pattern=7<<2;sprites[2].pattern=17<<2;
			sprites[0].colour = 1;sprites[2].colour = 15;
			sprites[1].pattern=8<<2;sprites[3].pattern=18<<2;
		...
		if (workflow==W_NEXTLEVEL)
		{
			...
			for (i=13;i<21;i++) sprites[i].y = RESERVED;			*/
			doInit();
			// Winpose 1 = pose de victoire
			/*if ((currentLevel&1)==0)*/ LoadPatternAndshowAScreenDan0(WINPOSE1_PATTERNDAN0,WINPOSE1_COLORDAN0,WINPOSE1_NAMEDAN0,1);
			/*else LoadPatternAndshowAScreenDan0(WINPOSE2_PATTERNDAN0,WINPOSE2_COLORDAN0,WINPOSE2_NAMEDAN0,1);*/
			screen_off();
			disable_nmi();
			//center_string(15,"NEXT LEVEL !!");
			center_string(17,"SCORE :");
			center_string(18,str(player.score));
			screen_on();
			enable_nmi();
			pressAKey();
			currentLevel++;
